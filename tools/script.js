/**
 * Mahdi Yasser — Tools & Extras Page
 * v11 design system: canvas, cursor, reveal + all original card/modal logic
 */
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    /* =========================================================
       5. DYNAMIC CARD LOADING (original logic, updated markup)
       ========================================================= */
    ...
    fetch(DATA_URL)
        ...
        .then(data => {
            ...
            // Re-attach cursor hover to newly created cards
            attachCursorHover('.link-card, .non-clickable-card, .card-button');

            // Attach card tilt effect
    ...
    /* =========================================================
       6. MODAL CLOSE LOGIC (original, untouched)
       ========================================================= */
    function closeModal() {
    ...
    if (closeStoryModalButton) {
        closeStoryModalButton.addEventListener('click', closeModal);
    }
    ...
    document.addEventListener('keydown', e => {
        if (e.key === 'Escape' && storyModalOverlay?.classList.contains('active')) {
            closeModal();
        }
    });
